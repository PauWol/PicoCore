...
class Config:
    ...
    def __init__(self, path):
        self.path = path
        self.file_object = {}     # structure: { "Class": {key:val, "sub": { ... }}}
        self.current_class = (None, None, False)  # (class, subclass, is_sub)
        self.subclasses = {}      # record of subclasses per class -> set(...)
        self._parse()
    ...
    def _convert_value(self, value):
        ...
        if v.startswith("{") and v.endswith("}"):
            try:
                ...
                return v  # fallback to raw string
    ...
    def _value_to_string(self, value):
        ...
        if isinstance(value, list):
            items = ", ".join(self._value_to_string(i) for i in value)
            return f"[{items}]"
    ...
    def _load(self):
        try:
            ...
            # file missing -> treat as empty config (safer for embedded)
            return ""
